...
/*
*In the below Dynamic Nat example (PAT- Multiple global ip address with pool of port) is Implemented.
* There are 6 private nodes among them , for first 4 nodes nat rule is being created.
*for 5th and 6th node rule doesn't exist so the packets are dropped by nat  and so , are not recieved by Server.
*each node is sending packets 2 times in this example this reveals the mapping done by Nat.
*/
...
// n0 <--------------------> n1 <-----------------------> n2
// 192.168.1.1   192.168.1.2    202.101.43.4  203.82.48.2
//
// Desired topology:
//       n0    n1   n2   n3   n4    n5    
//192.168.1.1 1.2   1.3  1.4  1.5  1.6
//       |     |    |    |    |    |
//       =========================== <-------------------> n6 <--------------------------------------------> n7
//                                           192.168.1.7         (202.101.43.4 - 202.101.43.5)            203.82.48.2
//                                                               (49153 - 49154) 
...
//each node is sending packets twice which gives clear picture of nat
...
//As Nat is out of global addresses and pool packets from node n4 & n5 are dropped.
  clientApps = echoClient.Install (nodes.Get (4));
  clientApps.Start (Seconds (2.0));
  clientApps.Stop (Seconds (10.0));

  clientApps = echoClient.Install (nodes.Get (5));
  clientApps.Start (Seconds (2.0));
  clientApps.Stop (Seconds (10.0));
